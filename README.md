...
Зачастую стоит задачу применения нескольких алгоритмов одновременно, например
...
или еще более коротко, использовав [конвейер, наподобии того как это принято в unix-системах](https://en.wikipedia.org/wiki/Pipeline_(Unix))
...
Все вышеуказанные сущности должна быть покрыты тестами, с помощью фреймворка [Google Test](http://google.github.io/googletest).

Тесты также являются частью задания, поэтому покрытие будет влиять на максимальный балл.

## NB

1. При реализации классов, для их использования в `range-based for`, он должен удовлетворять ряду [требований](https://en.cppreference.com/w/cpp/language/range-for).

2. Решаема я вами задача, является упрощенной реализацией библитеки std::ranges. Ее использование в данной работе запрешено, однако вы можете с ней ознакомиться и почерпнуть ряд идей вашей реализации
